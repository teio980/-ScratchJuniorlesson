...
if (!empty($keywords)) {
    ...
    $sql = "SELECT * FROM questions WHERE id like :keywords";
...
<body>
    <div class="search-container">
        <form action="" method="get">
            ...
            <?php endif; ?>
            <input type="text" name="query" id="searchInput" placeholder="Search Question ID..."  value="<?php echo htmlspecialchars($_SESSION['search_query']); ?>" required>
            <button type="submit" class="search-button" name="search">
                <span class="material-symbols-outlined">search</span>
            </button>
        </form>
        <a href="quiz.php?clear_search=1" class="clear_search"><span class="material-symbols-outlined">close</span></a>
    </div>
                ...
                <?php if ($records_per_page === 'ALL'): ?>
                    <?php foreach ($dataset as $data): ?>
                        <tr>   
                            ...
                            <td><?php echo htmlspecialchars($data['difficult']) ?></td>
                            <td><?php echo htmlspecialchars($data['option1']) ?></td>
                            <td><?php echo htmlspecialchars($data['option2']) ?></td>
                            <td><?php echo htmlspecialchars($data['option3']) ?></td>
                            <td><?php echo htmlspecialchars($data['option4']) ?></td>
                ...
                <?php else: ?>
                    <?php for ($i = $start; $i < $start + $records_per_page && $i < count($dataset); $i++): ?>
                        <tr>   
                            ...
                            <td><?php echo htmlspecialchars($dataset[$i]['question']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['difficult']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['option1']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['option2']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['option3']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['option4']) ?></td>
                            <td><?php echo htmlspecialchars($dataset[$i]['answer']) ?></td>
                        </tr>
                    <?php endfor; ?>
                <?php endif; ?>
            <?php else: ?>
                <tr>
                    <td colspan="8">Do Not Quiz Qustion at Now.</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table> 
    ...
    function printTable() {
        ...
        printWindow.document.write('<h1 style="text-align:center;">Quiz Question Table</h1>');
