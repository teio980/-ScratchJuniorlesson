...
if (!empty($keywords)) {
    $get_T_Materials_Sql = "SELECT * FROM teacher_materials WHERE title LIKE :keywords ";
    $get_T_Materials_Stmt = $pdo->prepare($get_T_Materials_Sql);
    $get_T_Materials_Stmt->bindValue(':keywords', '%' . $keywords . '%');
...
<body>
    <div class="search-container">
        <form action="" method="get">
            ...
            <?php endif; ?>
            <input type="text" name="query" id="searchInput" placeholder="Search Tittle..." value="<?php echo htmlspecialchars($_SESSION['search_query']); ?>">
            <button type="submit" class="search-button">
                <span class="material-symbols-outlined">search</span>
            </button>
        </form>
        <a href="teaching_materials.php?clear_search=1" class="clear_search"><span class="material-symbols-outlined">close</span></a>
    </div>
        ...
        <table>
            ...
            <?php if (!empty($T_Materials)): ?>
                ...
                if ($records_per_page === 'ALL') {
                    ...
                    for ($i = $start; $i < $start + $records_per_page && $i < count($T_Materials); $i++): ?>
                        <tr>
                            <td><?= htmlspecialchars($T_Materials[$i]['material_id']) ?></td>
                            <td><?= htmlspecialchars($T_Materials[$i]['teacher_id']) ?></td>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="7">Not Have Any Teaching Materials Now.</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
...
function printTable() {
    ...
    printWindow.document.write('<h1 style="text-align:center;">Teaching Materials Table</h1>');
...
function saveAsPDF() {
    ...
    const opt = {
        margin: 10,
        filename: 'teaching_materials.pdf',
