...
if (!empty($keywords)) {
    ...
    $Sql = "SELECT * FROM student_game_progress WHERE student_id LIKE :keywords ";
    $Stmt = $pdo->prepare($Sql);
    $Stmt->bindValue(':keywords', '%' . $keywords . '%');
    $Stmt->execute();
    $S_Game_Progress = $Stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    $Sql = "SELECT * FROM student_game_progress";
    $Stmt = $pdo->prepare($Sql);
    $Stmt->execute();
    $S_Game_Progress = $Stmt->fetchAll(PDO::FETCH_ASSOC);
}

$total_items = count($S_Game_Progress);
...
<body>
    <div class="search-container">
        ...
        <a href="student_puzzle_progress.php?clear_search=1" class="clear_search"><span class="material-symbols-outlined">close</span></a>
    </div>
        ...
        <table>
            ...
            <?php if (!empty($dataset)): ?>
                ...
                if ($records_per_page === 'ALL') {
                    ...
                    for ($i = $start; $i < $start + $records_per_page && $i < count($S_Game_Progress); $i++): ?>
                        <tr>
                            <td><?= htmlspecialchars($S_Game_Progress[$i]['progress_id']) ?></td>
                            <td><?= htmlspecialchars($S_Game_Progress[$i]['game_id']) ?></td>
                            <td><?= htmlspecialchars($S_Game_Progress[$i]['student_id']) ?></td>
                            <td><?= htmlspecialchars($S_Game_Progress[$i]['created_at']) ?></td>
                        </tr>
                    <?php endfor;
                } ?>
            <?php else: ?>
